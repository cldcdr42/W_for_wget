 ...
 ## Содержание
- [Счетчик успешного развертывания по гайду](#Счетчик-успешного-развертывания-по-гайду)
...
---
 ...
 # Счетчик успешного развертывания по гайду
 > Известные мне на текущий момент успешные попытки развертывания: 3  
...
8) Когда все готово, можно демонстрировать работу программы. Для этого  
   ...
   Прокидывание портов не является обязательным шагом для отправки curl запроса. Вместо этого достаточно найти айпи своего приложения (ns-<xx> заменяем на свой):
   ```
   kubectl get svc -n ns-<xx> -o wide
   ```
   Там будет сервис бд и приложения. Нам нужен айпи приложения. Запиште его  
   
   Далее выполните запрос. Вместо <pythoh-app-svc-IP> подставьте найденный IP адрес. В скобках '{"number": 1}' вместо 1 можно выбрать любое натуральное число от 1 до 100 [1;99].
   ```
   curl -X POST -H "Content-Type: application/json" -d '{"number": 1}' http://<pythoh-app-svc-IP>:5000/process
   ```
...
3) Записать все ipv4 (Например, так {у вас могут отличаться}): 54.194.23.65. 54.211.124.285, 54.223.15.189). Айпи с буквами (abcf) игнорировать
...
## Важно: после ребута системы правила перенаправления трафика докерхаба перестают работать, повторите процесс снова
...
Ключевые изменения:     
1) Для бд: указать свои переменные среды, образ используемой бд из докерхаба
