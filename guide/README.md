...
8) Когда все готово, можно демонстрировать работу программы. Для этого  
   ...
   ```
   Там будет сервис бд и приложения. Нам нужен айпи приложения. Запииште его  
   
   Далее выполните запрос. Вместо <pythoh-app-svc-IP> подставьте найденный адрес. В скобках '{"number": 1}' вместо 1 можно выбрать любое натуральное число от 1 до 100 [1;99].
   ```
