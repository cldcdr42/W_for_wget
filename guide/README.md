...
В случае использования собственного кода, как правило, изменения заключаются в изменении yml-файлов:

Само собой подразумевается, что namespace везде меняется на свой, имена деплоям, сервисам и ингресам задаются такие, которые удобны для вас. Ключевые изменения:  
1) Для бд: указать свои переменные среды, образ своей бд из докерхаба
2) Для приложение: указать свои переменные среды, свой образ из докерхаба, указать свои порты (которые использует приложение для кода)
...
1) бд postgres (postgres.yml): [GitHub URL](https://github.com/cldcdr42/W_for_wget/blob/main/apps/postgress/temp2-pg.yml)
2) приложения на пайтоне (flask) (python.yml): [GitHub URL](https://github.com/cldcdr42/W_for_wget/blob/main/apps/postgress/temp2-py.yml)
3) Ингрес (ingress.yml): [GitHub URL](https://github.com/cldcdr42/W_for_wget/blob/main/apps/postgress/ing.yml)
